...
class InboxKitten(TempMailProvider):
    def __init__(self):
        self.base_url = "https://api.inboxkitten.io"
    ...
    def create_email(self) -> tuple[str, str]:
        try:
            response = self.client.get(f"{self.base_url}/new")
            if response.status_code == 200:
                data = response.json()
                self.current_email = data.get("email", "")
                return self.current_email, ""
        except Exception as e:
            print(f"[InboxKitten] Create email error: {e}")
        return "", ""
    ...
    def get_messages(self, email: str) -> List[Dict]:
        if not email:
            return []
        try:
            response = self.client.get(f"{self.base_url}/messages/{email}")
            if response.status_code == 200:
                return response.json()
        except Exception as e:
            print(f"[InboxKitten] Get messages error: {e}")
        return []
    ...
    def get_verification_code(self, email: str, subject_contains: str = "", max_wait: int = 120) -> Optional[str]:
        ...
        while time.time() - start_time < max_wait:
            ...
            for msg in messages:
                ...
                code = self._extract_code(msg.get("text", "") or msg.get("html", ""))
                if code:
                    return code
            time.sleep(5)
        return None
