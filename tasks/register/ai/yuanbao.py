...
class YuanbaoRegister(BaseTask):
    def __init__(self, config: TaskConfig, global_config: Dict):
        super().__init__(config, global_config)
        self.signup_url = global_config.get("ai_services", {}).get("yuanbao", {}).get("signup_url", "https://yuanbao.tencent.com")
    ...
    def execute(self) -> TaskResult:
        ...
        try:
            ...
            with Camoufox(**launch_opts) as browser:
                ...
                try:
                    self.logger.info("Navigating to Yuanbao signup page...")
                ...
                return TaskResult(
                    ...
                    message=f"Yuanbao registration page opened: {email}",
                    data={"email": email}
                )
                
        except Exception as e:
            self.logger.error(f"Yuanbao registration failed: {str(e)}", e)
            self.take_error_screenshot()
            return TaskResult(
                task_id=self.config.task_id,
