...
class FakeMailProvider(EmailProvider):
    def __init__(self, api_url: str = "https://fakemail.top", timeout: int = 30):
    ...
    def get_messages(self, email: str) -> List[EmailMessage]:
        ...
        try:
            response = client.get("/api/mails", params={"email": email})
            
            if response.status_code == 200:
                data = response.json()
                messages = []
                for msg in data.get("mails", []):
    ...
    def get_message(self, email: str, message_id: str) -> Optional[EmailMessage]:
        client = self._get_client()
        try:
            response = client.get("/api/mail", params={"email": email, "id": message_id})
