...
class StepRegister(BaseTask):
    def __init__(self, config: TaskConfig, global_config: Dict):
        super().__init__(config, global_config)
        self.signup_url = global_config.get("ai_services", {}).get("step", {}).get("signup_url", "https://step.com")
    ...
    def execute(self) -> TaskResult:
        ...
        try:
            ...
            with Camoufox(**launch_opts) as browser:
                ...
                try:
                    self.logger.info("Navigating to Step signup page...")
                ...
                return TaskResult(
                    ...
                    message=f"Step registration page opened: {email}",
                    data={"email": email}
                )
                
        except Exception as e:
            self.logger.error(f"Step registration failed: {str(e)}", e)
            self.take_error_screenshot()
            return TaskResult(
                task_id=self.config.task_id,
