...
class KimiRegister(BaseTask):
    def __init__(self, config: TaskConfig, global_config: Dict):
        super().__init__(config, global_config)
        self.signup_url = global_config.get("ai_services", {}).get("kimi", {}).get("signup_url", "https://kimi.moonshot.cn")
    ...
    def execute(self) -> TaskResult:
        ...
        try:
            ...
            with Camoufox(**launch_opts) as browser:
                ...
                try:
                    self.logger.info("Navigating to Kimi signup page...")
                    self._page.goto(self.signup_url, timeout=30000)
                    self._page.wait_for_load_state("domcontentloaded")
                    time.sleep(2)
                    self.take_screenshot("01_signup_page")
                    
                    self._page.get_by_role("button", name="注册").click()
                    time.sleep(1)
                    
                    self._page.get_by_role("textbox", name="邮箱").fill(email)
                    self._page.get_by_role("button", name="验证").click()
                    time.sleep(2)
                ...
                return TaskResult(
                    ...
                    message=f"Kimi registration started: {email}",
                    data={"email": email}
                )
                
        except Exception as e:
            self.logger.error(f"Kimi registration failed: {str(e)}", e)
            self.take_error_screenshot()
            return TaskResult(
                task_id=self.config.task_id,
