...
class ZhipuaiRegister(BaseTask):
    def __init__(self, config: TaskConfig, global_config: Dict):
        super().__init__(config, global_config)
        self.signup_url = global_config.get("ai_services", {}).get("zhipuai", {}).get("signup_url", "https://open.bigmodel.cn")
    ...
    def execute(self) -> TaskResult:
        ...
        try:
            ...
            with Camoufox(**launch_opts) as browser:
                ...
                try:
                    self.logger.info("Navigating to ZhipuAI signup page...")
                ...
                return TaskResult(
                    ...
                    message=f"ZhipuAI registration page opened: {email}",
                    data={"email": email}
                )
                
        except Exception as e:
            self.logger.error(f"ZhipuAI registration failed: {str(e)}", e)
            self.take_error_screenshot()
            return TaskResult(
                task_id=self.config.task_id,
